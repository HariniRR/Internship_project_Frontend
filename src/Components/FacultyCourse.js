...
const FacultyCourse = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://localhost:3001/api/getFacultyCourse");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                const response = await axios.put(`http://localhost:3001/api/updateFacultyCourse/${editingId}`, formattedData);
                const updatedData = submittedData.map(data => data._id === editingId ? response.data : data);
                setSubmittedData(updatedData);
                setEditingId(null); // Reset editing ID
            } else {
                const response = await axios.post("http://localhost:3001/api/newFacultyCourse ", formattedData);
    ...
    return (
        <div className="container">
            <h4 className="mt-3 text-primary">Faculty Online Course Details</h4>
